...
def summarization(folder):
    ...
    for file_name in folder:
        ...
        outputs = model.generate(inputs,max_length=400,min_length= 200,length_penalty=2.0,num_beams=7,early_stopping=True)
        outputs = tokenizer.decode(outputs[0])

        if not os.path.isdir('notes_from_article/' + fine_label):
            os.mkdir('notes_from_article/' + fine_label)
        with open('notes_from_article/' + fine_label + '/'+ str(file_name[3:]), 'w') as fp:
...
if __name__ =='__main__':
    with open('original_articles/P1/P1_S01.json', 'r') as f:
        data = json.load(f)
    folder = generate_folders('P1_edited')
